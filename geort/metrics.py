...
def evaluate_trajectory(frames, qpos, joint_names, latencies, robot, hand):
    frames = list(frames)
    qpos = np.asarray(qpos)
    latencies = np.asarray(latencies, dtype=np.float64)
    expected_names = tuple(robot["joint_order"])
    if tuple(joint_names) != expected_names:
        raise ValueError("joint_names must exactly match the robot configuration")
    ...
    for index in np.flatnonzero(valid_frames):
        ...
        if robot_tips.shape != human_tips.shape or not np.isfinite(
            robot_tips
        ).all():
