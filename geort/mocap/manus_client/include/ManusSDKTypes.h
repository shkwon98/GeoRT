...
#define MAX_NUMBER_OF_DONGLES                                                                                               \
    16 // we only have 35 radio channels available. if we got 16 dongles, expect wireless issues! and no 16 usb ports is also
       // not likely.
...
/// @brief Used to descriptively refer to the max number of fingers we support per hand in chains (not the same as a glove,
/// as we can retarget alien hands with more or less fingers). Used with arrays and loops to make them more descriptive than
/// simply using the number, and to make changing the number easier and safer.
#define MAX_NUM_FINGER_IDS 10

/// @brief Used to descriptively refer to the max number of toes we support per foot in chains ( we can retarget alien feet
/// with more or less toes). Used with arrays and loops to make them more descriptive than simply using the number, and to
/// make changing the number easier and safer.
...
#define MAX_NUM_CHARS_IN_SKELETON_NAME                                                                                      \
    256 // we already encountered 34 char names in unreal, but its utf8 so enbiggen even more!
...
#define DEFAULT_BUFFER_SIZE         512
    ...
    /// @brief Describes the settings that can be applied to a node, it is defined as a flag so that more than one setting
    /// can be set.
    ...
    /// The transform is defined as a local or global transform depending on the coordinate system set when initializing the
    /// SDK. See functions CoreSdk_InitializeCoordinateSystemWithVUH and CoreSdk_InitializeCoordinateSystemWithDirection.
    ...
    // Skeleton
    // ------------------------------------------------------------------------------------------------------------------------------
    /// @brief Stores the inverse kinematics settings of a node, the ikAim is used to control the IK solve, 1 is default, -1
    /// is inversed.
    ...
    /// @brief Stores the settings for a node of type foot, heightFromGround is used to set the height of the 3d model ankle
    /// from ground.
    ...
    /// @brief Stores the settings of a node of type leaf, the direction vector is defined with respect to the previous node
    /// in the chain.
    ...
    /// @brief Stores the node setup information. Each node represents a segment of the skeleton that can be animated, nodes
    /// combine together to form chains. the parentID is used to identify the node with respect to which the current one will
    /// move.
    ...
    /// @brief Stores the information regarding the coordinate system used by the client, defined as VUH (view, up,
    /// handedness).
