...
{
    std::cerr << "Termination signal sent with parameter {}." << p_Parameter << std::endl;
...
{
    std::cerr << "Interrupt signal sent with parameter {}." << p_Parameter << std::endl;
...
{
    std::cerr << "Quit signal sent with parameter {}." << p_Parameter << std::endl;
...
{
    std::cerr << "Hang-up signal sent with parameter {}." << p_Parameter << std::endl;
...
{
    {
        const __sighandler_t t_OldTerminationHandler = signal(SIGTERM, HandleTerminationSignal);
    ...
    {
        const __sighandler_t t_OldInterruptHandler = signal(SIGINT, HandleInterruptSignal);
    ...
    {
        const __sighandler_t t_OldQuitHandler = signal(SIGQUIT, HandleQuitSignal);
    ...
    {
        const __sighandler_t t_OldHangupHandler = signal(SIGHUP, HandleHangupSignal);
...
/*static*/ bool SDKClientPlatformSpecific::CopyString(char *const p_Target, const size_t p_MaxLengthThatWillFitInTarget,
                                                      const std::string &p_Source)
{
    if (!p_Target)
    {
        std::cerr << "Tried to copy a string, but the target was null. The string was \"{}\"." << p_Source.c_str()
                  << std::endl;
    ...
    {
        std::cerr << "Tried to copy a string, but the target's size is zero. The string was ." << p_Source.c_str()
                  << std::endl;
    ...
    {
        std::cerr << "Tried to copy a string that was longer than characters, which makes it too big for its target buffer. "
                     "The string was "
                  << p_MaxLengthThatWillFitInTarget << p_Source.c_str() << std::endl;
...
std::string SDKClientPlatformSpecific::GetDocumentsDirectoryPath_UTF8(void)
{
    ...
    const std::string t_DocumentsDir =
        (!t_Xdg || strlen(t_Xdg) == 0) ? std::string(t_Home) + std::string("/Documents") : std::string(t_Xdg);

    return t_DocumentsDir;
}

std::ifstream SDKClientPlatformSpecific::GetInputFileStream(std::string p_Path_UTF8)
{
    return std::ifstream(p_Path_UTF8, std::ifstream::binary);
}

std::ofstream SDKClientPlatformSpecific::GetOutputFileStream(std::string p_Path_UTF8)
{
...
void SDKClientPlatformSpecific::CreateFolderIfItDoesNotExist(std::string p_Path_UTF8)
{
