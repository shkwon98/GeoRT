...
ClientReturnCode SDKMinimalClient::InitializeSDK()
{
    ...
    t_Direction.x = AxisDirection::AxisDirection_Forward; // X-axis points forward in ROS.
    t_Direction.y = AxisDirection::AxisDirection_Left;    // Y-axis points to the left in ROS.
    t_Direction.z = AxisDirection::AxisDirection_Up;      // Z-axis points up in ROS.
...
/// Callbacks that are registered functions that get called when a certain 'event' happens, such as data coming in from Manus
/// Core. All of these are optional, but depending on what data you require you may or may not need all of them.
ClientReturnCode SDKMinimalClient::RegisterAllCallbacks()
{
    ...
    const SDKReturnCode t_RegisterSkeletonCallbackResult =
...
{
    // ROS node setup
    auto node = std::make_shared<rclcpp::Node>("manus_tracker");
    tf2_ros::TransformBroadcaster broadcaster(node);

    auto headset_publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>("headset_tracker_data", 10);
    auto tracker1_publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>("tracker1_tracker_data", 10);
    auto tracker2_publisher = node->create_publisher<geometry_msgs::msg::PoseStamped>("tracker2_tracker_data", 10);

    // Create a static transform broadcaster
    tf2_ros::StaticTransformBroadcaster static_broadcaster(node);

    // Add a static frame at the origin
    geometry_msgs::msg::TransformStamped static_transform;
    static_transform.header.stamp = node->get_clock()->now();
    static_transform.header.frame_id = "base_link";       // Parent frame
    static_transform.child_frame_id = "lighthouse_frame"; // Child frame
    static_transform.transform.translation.x = -0.6;
    ...
    static_transform.transform.rotation.z = 1.0;
    ...
    while (m_Running)
    {
        // Check if there is new tracker data
        m_TrackerMutex.lock();
        if (m_NextTrackerData != nullptr)
        {
            // Iterate through each tracker data and publish on corresponding topic
            for (const auto &trackerData : m_NextTrackerData->trackerData)
            {
                auto message = geometry_msgs::msg::PoseStamped();
                message.header.stamp = node->get_clock()->now();
                message.header.frame_id = "lighthouse_frame";

                // Set tracker position and orientation
                message.pose.position.x = trackerData.position.x;
                message.pose.position.y = trackerData.position.y;
                message.pose.position.z = trackerData.position.z;

                message.pose.orientation.x = trackerData.rotation.x;
                message.pose.orientation.y = trackerData.rotation.y;
                message.pose.orientation.z = trackerData.rotation.z;
                message.pose.orientation.w = trackerData.rotation.w;

                // Publish on individual topics based on tracker ID
                if (strcmp(trackerData.trackerId.id, "headset serial") == 0)
                {
                    headset_publisher->publish(message);
                }
                else if (strcmp(trackerData.trackerId.id, "LHR-DAE7C1A7") == 0)
                {
                    tracker1_publisher->publish(message);
                }
                else if (strcmp(trackerData.trackerId.id, "LHR-3C6C2141") == 0)
                {
                    tracker2_publisher->publish(message);
                }

                // Broadcast transforms
                geometry_msgs::msg::TransformStamped transformStamped;
                transformStamped.header.stamp = node->get_clock()->now();
                transformStamped.header.frame_id = "lighthouse_frame";

                // if (trackerData.trackerId.id == "LHR-DAE7C1A7"){

                // 	transformStamped.child_frame_id = "Right_tracker";
                // }
                // else if(trackerData.trackerId.id == "LHR-3C6C2141")
                // {
                // 	transformStamped.child_frame_id = "Left_tracker";
                // }
                // else if (trackerData.trackerId.id == "headset serial")
                // {
                // 	transformStamped.child_frame_id = "headset_tracker";
                // }

                transformStamped.child_frame_id = trackerData.trackerId.id;

                transformStamped.transform.translation.x = trackerData.position.x;
                transformStamped.transform.translation.y = trackerData.position.y;
                transformStamped.transform.translation.z = trackerData.position.z;

                transformStamped.transform.rotation.x = trackerData.rotation.x;
                transformStamped.transform.rotation.y = trackerData.rotation.y;
                transformStamped.transform.rotation.z = trackerData.rotation.z;
                transformStamped.transform.rotation.w = trackerData.rotation.w;
...
void SDKMinimalClient::LoadTestSkeleton()
{
    ...
    t_SKL.settings.skeletonTargetUserIndexData.userIndex =
        0; // just take the first index. make sure this matches in the landscape.
...
/// @param p_PosX X position of the node, this is defined with respect to the global coordinate system or the local one
/// depending on the parameter p_UseWorldCoordinates set when initializing the sdk,
/// @param p_PosY Y position of the node this is defined with respect to the global coordinate system or the local one
/// depending on the parameter p_UseWorldCoordinates set when initializing the sdk,
/// @param p_PosZ Z position of the node this is defined with respect to the global coordinate system or the local one
/// depending on the parameter p_UseWorldCoordinates set when initializing the sdk,
/// @param p_Name the name of the node setup
/// @return the generated node setup
NodeSetup SDKMinimalClient::CreateNodeSetup(uint32_t p_Id, uint32_t p_ParentId, float p_PosX, float p_PosY, float p_PosZ,
                                            std::string p_Name)
{
    NodeSetup t_Node;
...
bool SDKMinimalClient::SetupHandNodes(uint32_t p_SklIndex)
{
    ...
    ManusVec3 t_Fingers[t_NumFingers * t_NumJoints] = {
        ...
        // CreateManusVec3(0.011181f, 0.031696f, 0.000000f), //Index CMC joint // Note: we are not adding the matacarpal
        // bones in this example, if you want to animate the metacarpals add each of them to the corresponding finger chain.
        CreateManusVec3(
            0.011181f, 0.000000f,
            0.052904f), // Index MCP joint, if metacarpal is present: CreateManusVec3(0.000000f, 0.000000f, 0.052904f)
        ...
        CreateManusVec3(
            -0.011274f, 0.000000f,
            0.049802f), // Ring MCP joint, if metacarpal is present: CreateManusVec3(0.000000f, 0.000000f, 0.049802f),
        ...
        CreateManusVec3(
            -0.020145f, 0.000000f,
            0.047309f), // Pinky MCP joint, if metacarpal is present: CreateManusVec3(0.000000f, 0.000000f, 0.047309f),
    ...
    for (uint32_t i = 0; i < t_NumFingers; i++)
    {
        ...
        {
            t_Res = CoreSdk_AddNodeToSkeletonSetup(
                p_SklIndex, CreateNodeSetup(1 + t_FingerId + j, t_ParentID, t_Fingers[i * 4 + j].x, t_Fingers[i * 4 + j].y,
                                            t_Fingers[i * 4 + j].z, "fingerdigit"));
            if (t_Res != SDKReturnCode::SDKReturnCode_Success)
            {
...
bool SDKMinimalClient::SetupHandChains(uint32_t p_SklIndex)
{
    ...
    const ChainType t_FingerTypes[5] = { ChainType::ChainType_FingerThumb, ChainType::ChainType_FingerIndex,
                                         ChainType::ChainType_FingerMiddle, ChainType::ChainType_FingerRing,
...
void SDKMinimalClient::OnSkeletonStreamCallback(const SkeletonStreamInfo *const p_SkeletonStreamInfo)
{
    if (s_Instance)
    {
        ...
        for (uint32_t i = 0; i < p_SkeletonStreamInfo->skeletonsCount; i++)
        {
            ...
            CoreSdk_GetSkeletonData(i, t_NxtClientSkeleton->skeletons[i].nodes,
                                    t_NxtClientSkeleton->skeletons[i].info.nodesCount);
        }
...
// Function to print tracker data
void PrintTrackerData(const TrackerData &trackerData)
{
    std::cout << "Tracker ID: " << trackerData.trackerId.id << std::endl;
    std::cout << "Tracker Position: (" << trackerData.position.x << ", " << trackerData.position.y << ", "
              << trackerData.position.z << ")" << std::endl;
    std::cout << "Tracker Rotation: (" << trackerData.rotation.w << ", " << trackerData.rotation.x << ", "
              << trackerData.rotation.y << ", " << trackerData.rotation.z << ")" << std::endl;
    std::cout << "Tracker Quality: " << static_cast<int>(trackerData.quality) << std::endl;
    std::cout << std::endl;
}
...
void SDKMinimalClient::OnTrackerStreamCallback(const TrackerStreamInfo *const p_TrackerStreamInfo)
{
    if (s_Instance)
    {
        ...
        for (uint32_t i = 0; i < p_TrackerStreamInfo->trackerCount; i++)
        {
            ...
            // Print each tracker data
            PrintTrackerData(t_TrackerData->trackerData[i]);
        }
